...
func (g *group) Create(ctx *context.T, call rpc.ServerCall, perms access.Permissions, entries []groups.BlessingPatternChunk) error {
	...
	if perms == nil {
		...
		if len(blessings) == 0 {
			...
			// to return an error.
			return groups.ErrorfErrNoBlessings(ctx, "no blessings recognized; cannot create group Permissions")
		}
...
func (g *group) readModifyWrite(ctx *context.T, call security.Call, version string, fn func(gd *groupData, versionSt string) error) error {
	...
	return groups.ErrorfErrExcessiveContention(ctx, "gave up after encountering excessive contention; try again later")
}
